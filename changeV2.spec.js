...
describe('Change', () => {
  ...
  test('test single coin change', () => {
    const change = new Change();
    const result = change.calculate([25], 25);
    expect(result).toEqual([25]);
  });

  test('test impossible single coin change', () => {
    const change = new Change();
    const message = 'The total 26 cannot be represented in the given currency.';
    const test = () => { change.calculate([25], 26); };
  ...
  // // WITH VERSION 1 FOLLOWING TWO WOULD FAIL
